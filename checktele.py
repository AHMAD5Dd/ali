...
async def _(event):
    if ispay2[0] == "yes":
        ...
        choice = str(msg[1])
        trys = 0
await fifthon.send_message(event.chat_id, f"حسناً سأفحص نوع `{choice}` من اليوزرات على `{ch}` , بعدد `{msg[0]}` من المحاولات !")

@Bhthon.on(events.NewMessage(outgoing=True, pattern=r"\.حالة الصيد"))
        async def _(event):
            if ispay2[0] == "yes":
                if "on" in isclaim:
                    await bhthon.send_message(event.chat_id, f"الصيد وصل لـ({trys}) من المحاولات")
                elif "off" in isclaim:
